...
def _process_single_cluster_with_keywords(
    ...
    cluster_texts = [
        ...
        if (a.get("id") in article_ids) or (a.get("url") in article_ids)
    ]
    ...
    missing = [k for k in article_ids if k not in article_map]
    if missing:
        logger.warning(f"[CLUSTER_{cluster_id}] Missing {len(missing)} article keys. Example: {missing[:3]}")

    seen = set()
    articles_in_cluster = []
    for k in article_ids:
        art = article_map.get(k)
        if not art:
            continue
        if art.id in seen:
            continue
        seen.add(art.id)
        articles_in_cluster.append(art)
...
async def cluster_summary_endpoint(payload: ClusterSummaryRequest, response: Response):
    ...
    for art in payload.articles:
        ...
        # If article has a URL field, use it as the key in article_map for easier access during clustering  
        url = getattr(art_en, "url", None) or getattr(art_en, "link", None) or getattr(art_en, "source_url", None)
        if url:
            article_map[url] = art_en
    ...
    for art_summary in summarized_articles:
        ...
        if summary:
            ...
            art_obj = article_map.get(article_id)
            url = None
            if art_obj:
                url = getattr(art_obj, "url", None) or getattr(art_obj, "link", None) or getattr(art_obj, "source_url", None)
