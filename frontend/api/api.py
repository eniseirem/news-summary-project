...
# INFRA SETUP
# ---------------------------
@app.post("/_create_index/{index_name}")
def create_index(index_name: str):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create index {index_name}: {e}")

@app.post("/_bulk_ingest/{index_name}")
def bulk_ingest(index_name: str, items: List[Dict[str, Any]]):
    if not items:
    ...
    try:
        res = client.bulk(body=operations, index=index_name)
        if res["errors"]:
            # Basic error reporting
            errors = [item.get("index", {}).get("error", {}) for item in res["items"] if "error" in item.get("index", {})]
            raise HTTPException(status_code=500, detail={"message": "Bulk ingest had errors", "errors": errors[:5]})
        return {"status": "success", "items_ingested": len(items)}
    except Exception as e:
